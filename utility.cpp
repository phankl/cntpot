...
double s5(double x) {
  return heaviside(-x) + heaviside(x) * heaviside(1 - x) *
                             (1 - x * x * x * (6 * x * x - 15 * x + 10));
}
...
double gammaFunction(double h, double alpha,
...
double surfaceElementDistance(double h, double alpha, double xi, double eta,
...
double smoothCutoff(double r) {
  ...
  return heaviside(-tau) +
         heaviside(tau) * heaviside(1 - tau) * (1 - tau * tau * (3 - 2 * tau));
}
